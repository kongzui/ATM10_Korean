...
Ponder.registry((allthemods) => {
	...
	.scene('fission_mek','Mekanism 핵분열로', 'kubejs:fission_mek',
	...
	(scene, util) => {
			...
			scene.text(80, '모서리는 반드시 케이싱으로 만드세요.', [0, 4.5, 4.5]).placeNearTarget();
			...
			scene.text(80, '벽에는 케이싱이나 원자로 유리를 사용할 수 있습니다.', [0, 2.5, 2.5]).placeNearTarget();
			...
			scene.text(60, '내부에 핵분열 연료 집합체를 배치해 연료봉을 만드세요.', [2.5, 2.5, 2.5]).placeNearTarget();
			...
			scene.idle(10)
			scene.text(60, '각 연료봉 위에 제어봉 집합체를 배치하세요.', [2.5, 3.5, 2.5]).placeNearTarget();
			scene.idle(20);
			scene.addKeyframe();
			scene.idle(60);
