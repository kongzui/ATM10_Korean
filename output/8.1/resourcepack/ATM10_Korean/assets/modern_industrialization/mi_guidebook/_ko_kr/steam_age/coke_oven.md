...
첫 멀티블록을 위해, 코크스로 블록 자체를 만들어야 합니다. 그 외에 21개의 벽돌들과 3개의 *해치*들이 필요합니다: 아이템 입력 해치, 아이템 출력 해치, 유체 입력 해치
...
코크스로 블록은 *컨트롤러*의 역할을 합니다. 모든 멀티블록은 컨트롤러가 있지만, 여기에 직접 아이템을 넣는 등의 상호 작용은 불가능합니다. 모든 입력과 출력은 해치를 통해서 이루어집니다. 코크스로는 증기로 가동되기 때문에, 유체 입력 해치가 있어야 하고, 석탄을 넣을 아이템 입력 해치와 코크스를 받을 아이템 출력 해치가 필요합니다.
...
코크스로 블록을 우클릭했을 때 *모양 유효*라고 나온다면, 유체 입력 해치에 증기를 채우고 아이템 입력 해치에 석탄을 넣어주세요. 이제 코크스로가 작동하기 시작합니다!
