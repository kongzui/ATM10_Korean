...
if (!Platform.isClientEnvironment()) {
  ...
  if (bannedEntities instanceof $ArrayList) {
    ...
    NativeEvents.onEvent($EntityJoinLevelEvent, event => {
      if (bannedEntities.contains(event.entity.type)) {
        event.level.server.tell(Text.red("금지된 엔티티 제거: " + event.entity.type + " 위치: " + event.entity.blockPosition().toShortString()))
  ...
  if (!bannedList.isEmpty()) {
    ...
    PlayerEvents.loggedIn((event) => {
      let data = bannedList
      console.log("금지된 아이템: " + data)
      console.log("금지된 아이템 목록 전송 대상: " + event.player.username)
  ...
  if (!bannedBlockEntities.isEmpty()) {
	...
	NativeEvents.onEvent($LevelTickEvent$Pre, event => {
	  ...
	  while ((pos = blocksToRemove.poll()) != null) {
		  ...
		  event.level.server.tell("금지된 블록 엔티티 제거 위치: " + pos.toShortString())
	  }
    })
...
function sendMessageAndCancel(/** @type {$ItemClickedKubeEvent_} */event, type) {
  switch (type) {
    case "item":
      event.player.setStatusMessage(Text.red("이 서버에서는 이 아이템을 사용할 수 없습니다!"))
      event.cancel()
      break
    default:
      break
...
let addSignToPos = (/** @type {$ServerLevel_} */level, /** @type {$BlockPos_} */pos) => {
  ...
  let nbt = {is_waxed: 1, front_text: {has_glowing_text: 1, color: "black", messages: [namespace, path, '"이 서버에서"', '"금지됨"']}}
