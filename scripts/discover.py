...
VISIBLE_TEXT_MARKERS = (
    ...
    (
        "text_component",
        re.compile(r"Text\.(?:of|red|green|yellow|blue|white|gold|translatable)\s*\("),
    ),
    (
        "ui_message",
        re.compile(
            r"(?:setStatusMessage|statusMessage|\.tell|\.respond|setTitle|setSubtitle)\s*(?:=|\()"
        ),
    ),
...
def scan_jars(
        ...
        try:
            with zipfile.ZipFile(jar) as archive:
                for entry in archive.namelist():
                    ...
                    if match:
                        groups[match.group(1).lower()][match.group(2).lower()].append(
                            entry
                        )
                        ...
                        if paths:
                            ...
                            for entry in paths:
                                ...
                                if error:
                                    state = "error"
                                    error_text = (
                                        error_text + " | " + f"{entry}: {error}"
                                    ).strip(" |")
                                    errors.append(
                                        {
                                            "source_type": "jar_json",
                                            "source": jar.name,
                                            "entry": entry,
                                            "error": error,
                                        }
                    ...
                    language_rows.append(
                        {
                            ...
                            "error": " | ".join(
                                filter(
                                    None,
                                    (
                                        locale_data["en_us_error"][0],
                                        locale_data["ko_kr_error"][0],
                                    ),
                                )
                            ),
                        }
        ...
        inventory.append(
            {
                ...
                "has_en_us": has_en,
                "has_ko_kr": has_ko,
                "classification": "en_and_ko"
                if has_en and has_ko
                else "en_only"
                if has_en
                else "ko_only"
                if has_ko
                else "no_target_language_file",
                "error": jar_error,
            }
        )
    ...
    summary = {
        ...
        "language_json_errors": sum(
            error["source_type"] == "jar_json" for error in errors
        ),
        "jars_with_en_us": sum(bool(row["has_en_us"]) for row in inventory),
        "jars_with_ko_kr": sum(bool(row["has_ko_kr"]) for row in inventory),
        "jars_with_en_and_ko": sum(
            bool(row["has_en_us"] and row["has_ko_kr"]) for row in inventory
        ),
        "jars_with_en_without_ko": sum(
            bool(row["has_en_us"] and not row["has_ko_kr"]) for row in inventory
        ),
        "namespaces_with_en_us": sum(
            row["en_status"] != "missing" for row in language_rows
        ),
        "namespaces_with_ko_kr": sum(
            row["ko_status"] != "missing" for row in language_rows
        ),
    ...
    return inventory, language_rows, errors, summary
...
def scan_ftbquests(instance: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    summary = {
        ...
        "snbt_files": sum(1 for path in root.rglob("*.snbt") if path.is_file())
        if root.is_dir()
        else 0,
        "chapter_files": sum(1 for path in (quest_root / "chapters").glob("*.snbt"))
        if (quest_root / "chapters").is_dir()
        else 0,
...
def scan_kubejs(
    ...
    errors: list[dict[str, Any]] = []
    for path in sorted(
        root.glob("assets/*/lang/*.json"), key=lambda p: p.as_posix().lower()
    ):
    ...
    for path in sorted(root.rglob("*.js"), key=lambda p: p.as_posix().lower()):
        ...
        for number, line in enumerate(lines, 1):
            if not STRING_WITH_LETTERS_RE.search(line):
                continue
            categories = [
                name for name, pattern in VISIBLE_TEXT_MARKERS if pattern.search(line)
            ]
            if categories:
    ...
    summary = {
        ...
        "js_files": sum(1 for path in root.rglob("*.js") if path.is_file())
        if root.is_dir()
        else 0,
...
def scan_other_candidates(instance: Path) -> list[dict[str, Any]]:
    ...
    for name in roots:
        ...
        for path in root.rglob("*"):
            if not path.is_file() or "ftbquests" in {
            ...
            if (
                re.search(r"(?:^|/)(?:en_us|ko_kr)(?:\.|/)", lowered)
                or "/lang/" in lowered
                or re.search(
                    r"(?:language|localization|translation)", path.name, re.IGNORECASE
                )
            ):
                rows.append(
                    {
                        ...
                        "reason": "이름 또는 경로가 언어/현지화 패턴과 일치",
                    }
                )
...
def read_pack_version(instance: Path) -> dict[str, Any]:
    ...
    if not path.is_file():
        return {
            ...
            "version": "",
            "minecraft": "",
            "manifest_file_entries": 0,
            "error": "manifest.json 없음",
        }
    try:
        ...
        return {
            ...
            "manifest_file_entries": len(data.get("files", [])),
            "error": "",
        }
    except Exception as exc:
        return {
...
def main() -> int:
    ...
    write_csv(
        ...
        [
            ...
            "en_estimated_keys",
            "ko_estimated_keys",
            "shared_estimated_keys",
            "missing_in_ko_estimated",
            "ko_only_estimated",
        ],
        ftb_chapters,
    )
    write_csv(
        output / "kubejs_languages.csv",
        ["path", "namespace", "locale", "keys", "status", "error"],
        kube_langs,
    )
    write_csv(
        output / "kubejs_text_candidates.csv",
        ["path", "line", "category", "comment_like", "excerpt"],
        kube_candidates,
    )
    write_csv(
        output / "other_translation_candidates.csv",
        ["path", "bytes", "reason"],
        other_candidates,
    )
    write_csv(
        output / "errors.csv", ["source_type", "source", "entry", "error"], errors
    )
    ...
    summary = {
        ...
        "notes": [
            "원본 파일은 읽기만 했으며 JAR을 추출하거나 수정하지 않음",
