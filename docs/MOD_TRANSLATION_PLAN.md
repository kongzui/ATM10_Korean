...
선택한 범위만 관리하며, 이 문서 전체를 자동 실행하지 않는다. 추천 진행 순서는
[8.1 누적 업데이트 로드맵](ATM10_VERSION_TRANSLATION_UPGRADE_PLAN.md)을 따른다.
...
- 현재 output 상태: 보조 번역 실행 코드를 제외한 `8.1-stable.1`과 `7.1-stable.1`을 제공한다.
...
항목은 전체 자동 대조와 변경분 수동 검토로 완료 상태를 갱신한다. 같은 키·같은 영어의
검수 번역은 유지한다. 신규 모드 전체 번역은 호환판 후속 누적 업데이트로 분리한다. 실제 영향
목록은 `versions/8.1/reports/pack_comparison.md`의 이식 당시 비교 기록을 참고하고,
현재 남은 작업과 순서는 아래 표와 누적 업데이트 로드맵을 따른다.
...
|---|---|---:|---:|---|
| 1 | Auroral / `auroral` | 148 | 0 | 일반 언어 미작업, 전용 퀘스트 이식 완료. 작은 첫 누적 단위 |
| 2 | Neo Vitae / `neovitae` | 3,053 | 0 | 일반 언어 미작업, 전용 퀘스트 이식 완료. 가장 큰 신규 범위 |
...
| 8 | Invasive Optimizations / `invasiveopts` | 3 | 0 | 표시 경로 조사 후 필요한 키 번역 |
...
| 우선순위 | 네임스페이스 | 후보 | 다음 처리 |
|---|---|---:|---|
| 높음 | `allthetweaks` | 24 | 공통 진행 아이템과 다른 언어 파일의 제공 여부 확인 |
| 중간 | `dyenamicsandfriends` | 1,944 | 설치 연동·활성 리소스팩의 실제 블록부터 확인 |
| 낮음 | `modonomicon` | 137 | 다른 네임스페이스·예제 책·표시 UI 분류 |
| 낮음 | `patchouli` | 13 | 예제·테스트 책과 실제 사용 안내 구분 |
| 감사 | `apotheosis`, `apothic_attributes` | 1 + 1 | 두 `comment_id`의 비표시 메타데이터 여부 확인 |

이는 기존 프로젝트 산출물에도 없던 키다. 8.1 신규·변경분 검토 대상의 누락은 이전 감사에서
0개였으며, 위 후보를 버전업 때문에 잃어버린 번역으로 계산하지 않는다.

### 기존 감사 밖의 추가 조사 후보: 98개 네임스페이스

최초 설치 조사 CSV와 현재 output 파일 목록을 추가 대조했다. 신규 11개와 기존 output의
누락 2,120키 외에도, 영어 키가 있지만 독립 `ko_kr.json` 산출물이 없는 네임스페이스 98개가 있다.
[전체 목록과 조사 순위](../versions/8.1/reports/additional_namespace_backlog.md)를 별도로 정리했다.
이는 98개 모드의 미번역 확정이 아니다. 내장 한국어·다른 네임스페이스 번역·내부 설정을 먼저 확인한다.
Storage Delight, Utilitarian, Bridging Mod 등 콘텐츠·조작 후보 → FTB Library 등 UI 후보 →
라이브러리·내부 표시 조사 순으로 기존 누락 정리 단계에 포함한다. 이름은 네임스페이스와 현재
모드 메타데이터를 대조해 확정한다. 오래된 완료 이력을 지우고 다시 번역하는 방식은 쓰지 않는다.
...
- 보조 선택판 우선순위: EnderDrives 툴팁 → Mouse Tweaks 설정 → EnderDrives 채팅 → AppleSkin F3.
  [별도 설계·검증 조건](AUXILIARY_TRANSLATION_SCRIPTS.md)을 통과하기 전 기본 배포에 넣지 않는다.
- 아래 완료 목록은 과거 계열 검수 이력이다. 보조 번역까지 현재 동작한다는 뜻이 아니다.
