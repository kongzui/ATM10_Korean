...
- 현재 output 상태: `8.1-compat.2`의 Rhino 시작 오류 수정과 실행 검증을 완료했으며,
  사용자 요청으로 7.1에도 오류 수정만 반영한 `7.1-final.1`을 제공한다. 실제 적용은 사용자가 한다.
- 신규 모드 전체 번역과 기존 표현 개선은 후속 누적 업데이트이며 게임 화면 확인은 사용자 요청으로 보류
