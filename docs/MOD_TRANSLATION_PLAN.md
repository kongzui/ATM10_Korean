...
이 문서는 ATM10 7.1에서 번역할 모드를 찾고 선택하기 위한 분류·안내 문서다.
...
| 상태 | 의미 |
|---|---|
| 완료 | 프로젝트 기준의 전체 검토·검증·산출물 반영·적용 기록이 있음 |
...
영어 문구의 출처를 추측하지 않는다. JEI, Jade, 공통 라이브러리나 다른 모드 화면에서 보이는
문구라도 실제 소유 모드와 네임스페이스를 확인한 뒤 선택 범위에 포함할지 결정한다.

## 작업 방식

1. 사용자가 선택한 모드와 관련 애드온의 실제 설치 여부와 버전을 확인한다.
2. Git 기록과 완료 기록을 확인해 이미 완료된 범위를 분리한다.
3. 영어 원문, 기존 한국어, 프로젝트 용어집과 관련 가이드를 비교한다.
4. 본체 언어 파일뿐 아니라 관련 FTB Quests, KubeJS, 발전 과제와 가이드북을 조사한다.
5. 작업량이 크면 약 100~200개 언어 키, 가이드 한 묶음 또는 퀘스트 한 챕터 단위의 내부
   배치로 나눈다.
6. JSON·SNBT 문법, 키, 자료형, 자리표시자, 줄바꿈, 서식 코드와 가이드 참조를 검증한다.
7. 검증된 산출물을 `output/`에 반영하고 독립적인 완료 단위로 커밋한다.
8. Minecraft·Java 프로세스를 확인하고 저장소 적용 스크립트로 설정된 대상에 적용한다.
9. 적용 전후 해시와 계획 밖 변경을 확인하고 원본 JAR을 수정하지 않았는지 검사한다.
10. 선택한 작업이 끝나면 완료 내용을 보고하고, 사용자가 고르지 않은 모드로 넘어가지 않는다.

AI는 구현·커밋·적용·파일 검증까지 담당한다. 실제 게임 화면의 문구 잘림, 문맥과 렌더링은
사용자가 확인하며, 발견된 문제는 선택한 모드의 후속 수정으로 처리한다.

## 완료 기준

- 선택한 본체와 애드온의 영어 원문 전체를 검토함
- 기존 한국어의 의미·용어·검색성·자리표시자를 검수함
- 관련 FTB Quests와 KubeJS 조사 및 필요한 번역을 완료함
- 가이드가 있으면 본문과 참조 파일을 검증함
- 남은 미번역과 수동 검토 항목이 없거나 보류 사유를 기록함
- 문법·키·자료형·자리표시자·줄바꿈·서식 코드 검증을 통과함
- 검증된 산출물과 완료 기록을 프로젝트에 반영함
- 산출물을 설정된 적용 대상에 적용하고 해시 일치를 확인함
