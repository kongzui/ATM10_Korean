...
테이프 드라이브는 도구, 방어구, 마법이 부여된 장비처럼 **NBT 데이터가 많은 아이템**이나 기존 ME 드라이브의 종류 공간을 각각 차지하는 고유 태그 아이템을 처리하도록 설계된 강력한 AE2 호환 저장 셀입니다.

일반적인 AE2 드라이브와 달리 테이프 디스크의 바이트 사용량은 저장된 아이템의 실제 **NBT 크기**에 따라 동적으로 변하므로 시스템을 세밀하게 관리할 수 있습니다. 테이프 드라이브는 필터에 맞는 아이템을 우선 저장해야 한다고 AE2에 알리지 **않습니다**. ME 드라이브 우선순위를 사용하세요.
...
각 테이프 디스크에는 표준 형식이 아닌 NBT가 있거나, 방어구·도구이거나, 중첩할 수 없는 아이템만 저장할 수 있습니다.
...
테이프 디스크는 **NBT 데이터가 많은 아이템**을 우선하도록 설계되어, 장비나 한 개씩 보관하는 아이템이 기존 드라이브의 종류 공간을 차지하지 않게 저장하기에 적합합니다.
...
테이프 드라이브는 일반 드라이브가 어려움을 겪는 영역, 즉 거대한 드라이브의 종류 한도를 차지하는 아이템을 처리하는 데 뛰어납니다.
...
테이프 디스크는 **NBT 데이터가 많거나**, **중첩할 수 없거나**, **맞춤 설정된** 아이템을 위한 특수 저장소이며 일반 대량 저장소가 아닙니다.
...
| <ItemImage id="minecraft:splash_potion" />      | 효과가 있는 **물약**                 |
| <ItemImage id="minecraft:netherite_pickaxe" />  | 내구도가 감소한 **도구**                |
