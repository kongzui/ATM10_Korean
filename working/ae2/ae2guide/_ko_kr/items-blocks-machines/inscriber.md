...
회로 인쇄기는 회로와 [프로세서](processors.md)를 [프레스](presses.md)로 인쇄하고 여러 아이템을 가루로 분쇄합니다.
...
입력 버퍼 크기를 조절할 수 있습니다. 예를 들어 인벤토리 하나에서 많은 회로 인쇄기로 재료를 공급한다면 작은 버퍼를 사용하세요.
첫 번째 인쇄기만 64개로 가득 차고 나머지는 비는 대신 재료가 여러 인쇄기에 더 알맞게 분배됩니다.
...
* 회로 인쇄기는 아래 설명처럼 면별 입출력을 사용하거나, 어느 면에서든 모든 슬롯에 입력을 허용하고 내부 필터가 목적지를 정하게 할 수 있습니다.
    면별 입출력을 사용하지 않을 때는 위·아래 슬롯에서 아이템을 추출할 수 없습니다.
* 아이템을 인접한 인벤토리로 밀어내도록 설정할 수 있습니다.
* 입력 버퍼 크기를 조절할 수 있습니다. 큰 버퍼는 수동으로 재료를 넣는 독립형 회로 인쇄기에 적합하고,
...
![회로 인쇄기 GUI](../assets/diagrams/inscriber_gui.png) ![회로 인쇄기 면](../assets/diagrams/inscriber_sides.png)

A. **위쪽 입력**: 회로 인쇄기의 위쪽 면으로 접근합니다(이 슬롯에는 아이템을 넣고 꺼낼 수 있음).

B. **가운데 입력**: 왼쪽, 오른쪽, 앞쪽, 뒤쪽 면으로 넣습니다(이 슬롯에는 아이템을 넣을 수만 있고 꺼낼 수 없음).

C. **아래쪽 입력**: 회로 인쇄기의 아래쪽 면으로 접근합니다(이 슬롯에는 아이템을 넣고 꺼낼 수 있음).

D. **출력**: 왼쪽, 오른쪽, 앞쪽, 뒤쪽 면으로 꺼냅니다(이 슬롯에서는 아이템을 꺼낼 수만 있고 넣을 수 없음).

## 간단한 자동화

면별 입출력과 회전 기능을 활용하면 다음과 같이 회로 인쇄기를 반자동화할 수 있습니다.
...
면별 입출력을 사용하지 않을 때는 파이프로 회로 인쇄기에 바로 넣고 꺼내도 됩니다.

## 업그레이드

회로 인쇄기는 다음 [업그레이드](upgrade_cards.md)를 지원합니다.
