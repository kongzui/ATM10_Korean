...
지정한 공간과 공간 저장소 차원의 같은 크기 공간을 서로 *교환*하는 방식으로 작동합니다. 파일런 배열
안에 있던 것은 공간 저장소 차원으로 보내고, 해당 차원에 있던 것은 파일런 배열 안으로 가져옵니다.
...
파일런 안의 공간과 공간 저장소 차원의 공간이 서로 *교환*됩니다. 블록 묶음 하나를 공간 저장소 차원으로
보낸 다음 파일런 안에 다른 블록 묶음을 놓고 셀을 입력 슬롯에 다시 넣어 I/O 포트를 작동시키면, 두 번째
...
파일런 바깥쪽을 둘러싼 경계 상자에서 모든 방향으로 한 블록씩 안쪽으로 줄인 범위가 실제 공간입니다.

규칙은 다음과 같습니다.
- 최소 크기는 3x3x3이며, 이때 1x1x1 공간을 정의합니다.
- 모든 공간 파일런은 바깥쪽 경계 상자 위에 있어야 합니다.
- 모든 공간 파일런은 같은 네트워크에 있어야 합니다.
- 모든 파일런은 최소 두 블록 길이여야 합니다.

예를 들어 3x3x3 공간을 정의하려면 두 번째 규칙에 따라 모든 파일런이 지정할 공간 주변의 5x5x5 껍질
...
파일런 배열의 효율은 껍질을 얼마나 많이 채웠는지에 따라 달라집니다. 큰 공간 주변에 최소한의 파일런만
