...
현재 배포는 KubeJS 시작 오류를 고친 `8.1-compat.3`예요. 7.1 검수 번역을 보존하고
현재 버전의 변경분만 검토해 누적했어요. 신규 모드 전체 번역은 후속 업데이트로 분리했어요.
사용자의 명시적 요청으로 7.1 산출물에도 오류 수정을 반영해 `7.1-final.2`을 만들었어요.
이번 두 수정판은 사용자가 직접 설치하며 실제 인스턴스에는 적용하지 않았어요.
...
- compat.1의 선언 오류에 이어 compat.2의 getClass 화면 충돌을 수정했어요. compat.3은
  실제 Java 객체 래퍼를 사용하는 Rhino 검사를 통과했고 Minecraft 화면 재확인은 남아 있어요.
